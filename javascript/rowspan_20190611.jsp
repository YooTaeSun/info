...
var merge_obj = {
	idxObj :  {},
	fn_rowSum : function($trs, targetArray){
		console.log(" targetArray >> " + JSON.stringify(targetArray));
		var idxObj = this.idxObj;
			...
			$.each($tds, function(tdi,td) {
				...
				if(tdi == 0){//마직막 tr
					...
					if(btext == text){//
						sameCnt++;
						if(sameCnt >= 2){
							idxObj[val][totalCnt - tdi -1] = totalCnt - tdi -1 + sameCnt - 1;
						}
					}else{
						if(sameCnt >= 2){
							idxObj[val][totalCnt - tdi] = totalCnt - tdi + sameCnt - 1;
						}
					}
				}else{
					if(btext == text){
		...
		console.log("==============================================");
		this.fn_calculate_merge(idxObj);
	},
	fn_row_excute : function (key,val){
		console.log(" fn_row_merge val >> " + JSON.stringify(val));

		for ( var vi in val) {
			var gtIdx = parseInt(vi-1);
			var ltIdx = val[vi] + 1;
			var rowCnt = val[vi] - vi + 1;
			var rows = null;
	...
	putArray : function(next_array, start, end){
		console.log("start >> " + start +  ", end >> " + end);
		var obj = {};
		obj[start] = end;
		next_array.push(obj);
	},
	fn_calculate_merge : function(idxObj){
	  	var b_idxArray = [];
		var val = null;
		var start = -1;
		var end = -1;
		...
		for ( var key_idxObj in idxObj) {
			...
			if(b_idxArray.length > 0) {

				var next_array  = [];
				var cnt = Object.keys(el_obj).length;
				var count = 0;
				...
				for ( var key in el_obj) {
					...
					for (var i = 0; i < b_idxArray.length; i=i+2) {

						var p_start = b_idxArray[i];
						var p_end = b_idxArray[i+1];
...
<table class=" t_cetner" border="1">
	...
	<tr>
		...
		<td>
			<table class="table-style t_cetner" border="1" id="t1">
		<colgroup>
			<col style="width:60px;">
			<col style="width:60px;">
			<col style="width:30%;">
...
<table border="1">
					<colgroup>
						...
						<col style="width:60px;"><!-- 책임자 -->
						<col style="width:50px;"><!-- 관리자 -->
						<col style="width:60px;"><!-- 운영자 -->
						<col style="width:60px;"><!-- 부서평가 -->
						<col style="width:60px;"><!-- 내부평가 -->
						...
						<col style="width:200px;"><!-- 증빙자료 파일	 -->
						<col style="width:100px;"><!-- 증빙자료 등록일자 -->
						<col>
